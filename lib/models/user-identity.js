 ...
 */
 module.exports = function(UserIdentity) {
  ...
  function profileToUser(provider, profile, options, profilecb) {
  ...
  UserIdentity.login = function (provider, authScheme, profile, credentials,
    ...
    userIdentityModel.findOne({where: {
      ...
      (options.profileToUser || profileToUser)(provider, profile, options, function(err, userObj){
        ...
        // SECURITY UPDATE DO NOT ALLOW PASSPORT AUTH PARTY TO TAKEOVER EXISTING ACCOUNT
        userModel.findOne({ where: query }, userObj, function (err, userexists) {
          if (err) {
            return cb(err);
          }
          if(userexists){
            console.log(userexists);
            return cb('email/username already exists');
          }
          userModel.create(userObj, function (err, user) {
            if (err) {
              return cb(err);
            }
            var date = new Date();
